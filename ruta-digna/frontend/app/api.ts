...
export const getGuiaVisita = (visitaId: string) =>
  apiFetch<any[]>(`/guias/visita/${visitaId}`)

export const getEstudiosReordenables = (visitaId: string) =>
  apiFetch<any>(`/visitas/${visitaId}/estudios-reordenables`)
...
export const recomendar = (mensaje: string, lat?: number, lon?: number) =>
  apiFetch<RecomendacionResponse>('/ia/recomendar', {
    method: 'POST',
...
export const chatAsistente = (visita_id: string, mensaje: string, historial: any[] = []) =>
  apiFetch<{ reply: string }>('/ia/chat', {
    method: 'POST',
    body: JSON.stringify({ visita_id, mensaje, historial }),
  })

export const explicarResultados = (data: {
...
export const getRestricciones = (idEstudio: number, edad?: number, meses?: number) =>
  apiFetch<any>(`/estudios/${idEstudio}/restricciones?${
    new URLSearchParams({
      ...(edad !== undefined ? { edad_paciente: String(edad) } : {}),
      ...(meses !== undefined ? { meses_desde_ultimo: String(meses) } : {}),
    })
  }`)
